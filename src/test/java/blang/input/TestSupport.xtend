...
class TestSupport {
  ...
  def static void assertThrownExceptionMatches(Throwable expectedException, Runnable code) {
    assertThrownExceptionMatches(code, expectedException, true)
  }
  
  def static void assertTypeOfThrownExceptionMatches(Throwable expectedException, Runnable code) {
